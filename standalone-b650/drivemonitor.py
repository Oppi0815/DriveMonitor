...
def ensure_root() -> None:
    if os.environ.get("DRIVEMONITOR_TEST_MODE") == "1":
        return
    if os.geteuid() == 0:
        return
...
def maintenance_user_home() -> Path:
    """Return the desktop user's home even when DriveMonitor runs as root."""
    for env_name in ("PKEXEC_UID", "SUDO_UID"):
        raw = os.environ.get(env_name)
        if raw and raw.isdigit():
            try:
                return Path(pwd.getpwuid(int(raw)).pw_dir)
    ...
    if sudo_user and sudo_user != "root":
        try:
            ...
            pass
    # The B650 installation belongs to Horst; use the only normal home if clear.
    homes = [p for p in Path("/home").iterdir() if p.is_dir()] if Path("/home").exists() else []
    return homes[0] if len(homes) == 1 else Path.home()
...
def maintenance_log(home: Path, action: str, result: str) -> None:
    log_dir = home / ".local" / "share" / "drivemonitor"
    log_dir.mkdir(parents=True, exist_ok=True)
    log_file = log_dir / "maintenance.log"
    with log_file.open("a", encoding="utf-8") as handle:
        handle.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')}\t{action}\t{result}\n")


class MaintenanceWindow(tk.Toplevel):
    """Safe, confirmation-based maintenance dashboard for the B650."""

    def __init__(self, master: tk.Misc) -> None:
        super().__init__(master)
        self.home = maintenance_user_home()
        self.title("DriveMonitor 4.6 – Systemwartung")
        self.configure(bg=BG)
        self.geometry("900x700")
        self.minsize(820, 610)
        self.transient(master)
        self.values: dict[str, tk.StringVar] = {}
        self.symbols: dict[str, tk.StringVar] = {}
        self.buttons: list[tk.Button] = []
        self._build()
        self.after(150, self.refresh_checks)

    def _build(self) -> None:
        header = tk.Frame(self, bg=BG)
        header.pack(fill="x", padx=18, pady=(14, 8))
        tk.Label(header, text="Systemwartung", font=("Sans", 20, "bold"),
                 bg=BG, fg=TEXT).pack(side="left")
        tk.Label(header, text="B650", font=("Sans", 9, "bold"),
                 bg=BG, fg=BLUE).pack(side="left", padx=(8, 0), pady=(5, 0))
        self.check_btn = tk.Button(header, text="Neu prüfen", command=self.refresh_checks,
                                   relief="flat", padx=12, pady=5, cursor="hand2",
                                   bg="#e7ecef", fg=TEXT)
        self.check_btn.pack(side="right")

        tk.Label(self, text="Prüfungen verändern nichts. Jede Bereinigung muss bestätigt werden.",
                 font=("Sans", 9), bg=BG, fg=MUTED).pack(anchor="w", padx=18, pady=(0, 10))

        summary = tk.Frame(self, bg=BG)
        summary.pack(fill="x", padx=18, pady=(0, 10))
        for i in range(3): summary.grid_columnconfigure(i, weight=1)
        self.health_var = tk.StringVar(value="Wird geprüft …")
        self.free_var = tk.StringVar(value="—")
        self.recommend_var = tk.StringVar(value="—")
        for i, (title, var) in enumerate((("Systemzustand", self.health_var),
                                          ("Freier Speicher /", self.free_var),
                                          ("Empfehlung", self.recommend_var))):
            card = tk.Frame(summary, bg=CARD, highlightthickness=1, highlightbackground=BORDER)
            card.grid(row=0, column=i, sticky="nsew", padx=(0 if i == 0 else 4, 0 if i == 2 else 4))
            tk.Label(card, text=title, font=("Sans", 8, "bold"), bg=CARD, fg=MUTED).pack(anchor="w", padx=10, pady=(7, 1))
            tk.Label(card, textvariable=var, font=("Sans", 13, "bold"), bg=CARD, fg=TEXT,
                     wraplength=240, justify="left").pack(anchor="w", padx=10, pady=(0, 8))
        ...
        tk.Label(body, text="Prüfergebnisse", font=("Sans", 11, "bold"), bg=CARD, fg=TEXT).pack(anchor="w", padx=12, pady=(10, 5))
        self.rows = tk.Frame(body, bg=CARD)
        self.rows.pack(fill="x", padx=10, pady=(0, 10))
        self.rows.grid_columnconfigure(2, weight=1)
        definitions = [
            ("trash", "Papierkorb", "Papierkorb leeren", self.empty_trash),
            ("downloads", "Downloads", "Ordner öffnen", self.open_downloads),
            ("cache", "Benutzer-Cache", "Cache-Auswahl", self.cache_dialog),
            ("pip", "Python-pip-Cache", "pip-Cache leeren", self.clear_pip),
            ("apt", "APT-Paketcache", "APT-Cache leeren", self.clean_apt),
            ("autoremove", "Verwaiste APT-Pakete", "APT autoremove", self.apt_autoremove),
            ("flatpak", "Flatpak", "Ungenutzte entfernen", self.flatpak_unused),
            ("journal", "Systemprotokolle", "Auf 14 Tage begrenzen", self.vacuum_journal),
        ]
        for row, definition in enumerate(definitions): self._row(row, *definition)

        footer = tk.Frame(self, bg=BG)
        footer.pack(fill="x", padx=18, pady=(0, 14))
        self.status_var = tk.StringVar(value="")
        tk.Label(footer, textvariable=self.status_var, font=("Sans", 9), bg=BG, fg=MUTED).pack(side="left")
        tk.Button(footer, text="Protokoll öffnen", command=self.open_log, relief="flat", padx=12, pady=6,
                  bg="#e7ecef", fg=TEXT).pack(side="right", padx=(8, 0))
        tk.Button(footer, text="Schließen", command=self.destroy, relief="flat", padx=12, pady=6,
                  bg="#e7ecef", fg=TEXT).pack(side="right")

    def _row(self, row: int, key: str, title: str, action: str, callback: Any) -> None:
        self.symbols[key] = tk.StringVar(value="…")
        self.values[key] = tk.StringVar(value="Wird geprüft")
        tk.Label(self.rows, textvariable=self.symbols[key], width=3, font=("Sans", 11, "bold"), bg=CARD, fg=MUTED).grid(row=row, column=0, pady=4)
        tk.Label(self.rows, text=title, width=22, anchor="w", font=("Sans", 9, "bold"), bg=CARD, fg=TEXT).grid(row=row, column=1, sticky="w", padx=3, pady=4)
        tk.Label(self.rows, textvariable=self.values[key], anchor="w", font=("Sans", 9), bg=CARD, fg=MUTED).grid(row=row, column=2, sticky="ew", padx=3, pady=4)
        button = tk.Button(self.rows, text=action, command=callback, width=21, relief="flat", cursor="hand2", bg="#eef2f4", fg=TEXT)
        button.grid(row=row, column=3, padx=(8, 4), pady=3)
        self.buttons.append(button)

    def _busy(self, state: bool, text: str = "") -> None:
        widget_state = "disabled" if state else "normal"
        self.check_btn.config(state=widget_state)
        for button in self.buttons: button.config(state=widget_state)
        self.status_var.set(text)

    def refresh_checks(self) -> None:
        self._busy(True, "System wird geprüft …")
        threading.Thread(target=self._collect, daemon=True).start()

    def _collect(self) -> None:
        root = shutil.disk_usage("/")
        trash = directory_size(self.home / ".local/share/Trash/files")
        downloads = directory_size(self.home / "Downloads")
        cache = directory_size(self.home / ".cache")
        pip_cache = directory_size(self.home / ".cache/pip")
        apt_cache = directory_size(Path("/var/cache/apt/archives"))
        _, apt_out, _ = run(["apt-get", "-s", "autoremove"], timeout=40)
        removable = sum(1 for line in apt_out.splitlines() if line.startswith("Remv "))
        flatpak = "Nicht installiert"
        if shutil.which("flatpak"):
            code, out, err = run(["flatpak", "uninstall", "--unused", "--assumeno"], timeout=60)
            low = (out + err).lower()
            if "nothing unused" in low or "nichts unbenutztes" in low:
                flatpak = "Keine ungenutzten Laufzeiten"
            elif "will be removed" in low or "werden entfernt" in low:
                flatpak = "Ungenutzte Komponenten gefunden"
            else:
                flatpak = "Prüfung verfügbar"
        journal = "Nicht verfügbar"
        if shutil.which("journalctl"):
            _, out, err = run(["journalctl", "--disk-usage"], timeout=20)
            lines = (out + err).strip().splitlines()
            if lines: journal = lines[-1]
        data = dict(root_free=root.free, root_used=round(root.used/root.total*100), trash=trash,
                    downloads=downloads, cache=cache, pip=pip_cache, apt=apt_cache,
                    autoremove=removable, flatpak=flatpak, journal=journal)
        self.after(0, lambda: self._apply(data))

    def _apply(self, data: dict[str, Any]) -> None:
        def setrow(key: str, value: str, warning: bool = False) -> None:
            self.symbols[key].set("!" if warning else "✓")
            self.values[key].set(value)
        setrow("trash", human_bytes(data["trash"]), data["trash"] >= 1024**3)
        setrow("downloads", human_bytes(data["downloads"]), data["downloads"] >= 50*1024**3)
        setrow("cache", human_bytes(data["cache"]), data["cache"] >= 10*1024**3)
        setrow("pip", human_bytes(data["pip"]), data["pip"] >= 2*1024**3)
        setrow("apt", human_bytes(data["apt"]), data["apt"] >= 1024**3)
        setrow("autoremove", f"{data['autoremove']} Pakete", bool(data["autoremove"]))
        setrow("flatpak", data["flatpak"], "gefunden" in data["flatpak"].lower())
        self.symbols["journal"].set("•"); self.values["journal"].set(data["journal"])
        warnings=[]
        if data["trash"] >= 1024**3: warnings.append("Papierkorb leeren")
        if data["pip"] >= 2*1024**3: warnings.append("pip-Cache leeren")
        if data["cache"] >= 10*1024**3: warnings.append("Cache prüfen")
        if data["downloads"] >= 50*1024**3: warnings.append("Downloads prüfen")
        if data["autoremove"]: warnings.append("APT autoremove")
        self.health_var.set("Gut" if len(warnings) < 2 else "Prüfen")
        self.free_var.set(f"{human_bytes(data['root_free'])} frei · {data['root_used']} % belegt")
        self.recommend_var.set(warnings[0] if warnings else "Keine dringende Aktion")
        self._busy(False, f"Zuletzt geprüft: {time.strftime('%H:%M:%S')}")

    def _confirm(self, title: str, text: str) -> bool:
        from tkinter import messagebox
        return messagebox.askyesno(title, text, parent=self)

    def _info(self, title: str, text: str, error: bool = False) -> None:
        from tkinter import messagebox
        (messagebox.showerror if error else messagebox.showinfo)(title, text, parent=self)

    def empty_trash(self) -> None:
        if not self._confirm("Papierkorb leeren", "Den persönlichen Papierkorb endgültig leeren?"): return
        removed=0
        for folder in (self.home/".local/share/Trash/files", self.home/".local/share/Trash/info"):
        ...
        maintenance_log(self.home, "Papierkorb leeren", f"{removed} Einträge entfernt")
        self._info("Papierkorb", f"{removed} Einträge entfernt.")
        self.refresh_checks()

    def clear_pip(self) -> None:
        path=self.home/".cache/pip"
        if self._confirm("pip-Cache leeren", f"{human_bytes(directory_size(path))} pip-Cache löschen?"):
            shutil.rmtree(path, ignore_errors=True); maintenance_log(self.home, "pip-Cache", "geleert"); self.refresh_checks()

    def cache_dialog(self) -> None:
        from tkinter import messagebox
        choices=[("Vorschaubilder", self.home/".cache/thumbnails"),
                 ("Mesa-Shadercache", self.home/".cache/mesa_shader_cache"),
                 ("Mesa-Shader-Datenbank", self.home/".cache/mesa_shader_cache_db"),
                 ("Mint-Softwareverwaltung", self.home/".cache/mintinstall"),
                 ("Tracker-Suchindex", self.home/".cache/tracker3")]
        dialog=tk.Toplevel(self); dialog.title("Cache-Auswahl"); dialog.configure(bg=BG); dialog.geometry("520x350"); dialog.transient(self)
        tk.Label(dialog, text="Sichere Cache-Bereinigung", font=("Sans", 15, "bold"), bg=BG, fg=TEXT).pack(anchor="w", padx=16, pady=(14,8))
        tk.Label(dialog, text="Browserprofile, E-Mails und Einstellungen bleiben erhalten.", bg=BG, fg=MUTED).pack(anchor="w", padx=16)
        vars=[]
        for label,path in choices:
            var=tk.BooleanVar(value=True); vars.append((var,path))
            tk.Checkbutton(dialog, text=f"{label} ({human_bytes(directory_size(path))})", variable=var, bg=BG, fg=TEXT, selectcolor=CARD).pack(anchor="w", padx=18, pady=3)
        def clean():
            selected=[p for v,p in vars if v.get()]
            if selected and messagebox.askyesno("Cache löschen", "Ausgewählte, neu erzeugbare Caches löschen?", parent=dialog):
                for p in selected: shutil.rmtree(p, ignore_errors=True)
                maintenance_log(self.home, "Cache-Auswahl", f"{len(selected)} Bereiche")
                dialog.destroy(); self.refresh_checks()
        tk.Button(dialog, text="Auswahl löschen", command=clean, relief="flat", padx=13, pady=7, bg="#e7ecef", fg=TEXT).pack(anchor="e", padx=16, pady=16)

    def _command_action(self, title: str, cmd: list[str], success: str) -> None:
        self._busy(True, f"{title} läuft …")
        def worker():
            code,out,err=run(cmd, timeout=240)
            result=success if code==0 else f"Fehler ({code}): {(out+err)[-500:]}"
            maintenance_log(self.home, title, result)
            self.after(0, lambda: self._command_done(title, result, code != 0))
        threading.Thread(target=worker, daemon=True).start()

    def _command_done(self, title: str, result: str, error: bool) -> None:
        self._info(title, result, error); self.refresh_checks()

    def clean_apt(self) -> None:
        if self._confirm("APT-Cache leeren", "Heruntergeladene APT-Installationspakete löschen?"):
            self._command_action("APT-Cache leeren", ["apt-get", "clean"], "APT-Paketcache wurde geleert.")

    def apt_autoremove(self) -> None:
        if self._confirm("APT autoremove", "Nicht mehr benötigte Abhängigkeiten entfernen?"):
            self._command_action("APT autoremove", ["apt-get", "autoremove", "-y"], "Nicht mehr benötigte Pakete wurden entfernt.")

    def flatpak_unused(self) -> None:
        if not shutil.which("flatpak"): self._info("Flatpak", "Flatpak ist nicht installiert."); return
        if self._confirm("Flatpak bereinigen", "Ungenutzte Flatpak-Laufzeiten entfernen?"):
            user_name = self.home.name
            cmd = ["runuser", "-u", user_name, "--", "flatpak", "uninstall", "--unused", "-y"] if os.geteuid()==0 else ["flatpak", "uninstall", "--unused", "-y"]
            self._command_action("Flatpak bereinigen", cmd, "Ungenutzte Flatpak-Komponenten wurden entfernt.")

    def vacuum_journal(self) -> None:
        if self._confirm("Systemprotokolle begrenzen", "Journal-Protokolle älter als 14 Tage entfernen?"):
            self._command_action("Journal begrenzen", ["journalctl", "--vacuum-time=14d"], "Systemprotokolle wurden auf 14 Tage begrenzt.")

    def open_downloads(self) -> None:
        user_name=self.home.name
        cmd=["runuser", "-u", user_name, "--", "xdg-open", str(self.home/"Downloads")] if os.geteuid()==0 else ["xdg-open", str(self.home/"Downloads")]
        subprocess.Popen(cmd)

    def open_log(self) -> None:
        log=self.home/".local/share/drivemonitor/maintenance.log"; log.parent.mkdir(parents=True, exist_ok=True); log.touch(exist_ok=True)
        user_name=self.home.name
        cmd=["runuser", "-u", user_name, "--", "xdg-open", str(log)] if os.geteuid()==0 else ["xdg-open", str(log)]
        subprocess.Popen(cmd)
...
class Dashboard(tk.Tk):
    def __init__(self) -> None:
        ...
        icon_path = Path(__file__).resolve().parent / "icons" / "drivemonitor-64.png"
        self._app_icon = None
        if icon_path.exists():
            try:
    ...
    def build_header(self) -> None:
        ...
        self.maintenance_btn = tk.Button(
            header, text="Systemwartung", command=self.open_maintenance,
            relief="flat", padx=12, pady=5, cursor="hand2",
            bg="#e7ecef", fg=TEXT, activebackground="#d9e1e5"
        )
        self.maintenance_btn.pack(side="right", padx=(0, 8))
        self.status = tk.Label(header, text="", font=("Sans", 9),
                               bg=BG, fg=MUTED)
        self.status.pack(side="right", padx=(0, 14))

    def open_maintenance(self) -> None:
        existing = getattr(self, "_maintenance_window", None)
        if existing is not None and existing.winfo_exists():
            existing.lift()
            existing.focus_force()
            return
        self._maintenance_window = MaintenanceWindow(self)
